...
export class InviteHistoryController {
  ...
  async check(options: InviteHistoryControllerOptions['check']): Promise<boolean | undefined> {
  ...
  async delete(options: InviteHistoryControllerOptions['delete']): Promise<boolean> {
  ...
  ): Promise<Page<InviteHistoryDto['InviteHistoryRepository/COMPLEX_FETCHER']>> {
    const _uri = '/inviteHistory/query'
    return (await this.executor({ uri: _uri, method: 'POST', body: options.body })) as Page<
      InviteHistoryDto['InviteHistoryRepository/COMPLEX_FETCHER']
    >
  }
