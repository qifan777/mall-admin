...
const { updateForm, dialogData } = storeToRefs(agentLevelStore)
const updateFormRef = ref<FormInstance>()
const rules = reactive<FormRules<typeof updateForm>>({})
const init = async () => {
  dialogData.value.title = '编辑'
  updateForm.value = {
    ...(await api.agentLevelController.findById({ id: updateForm.value.id || '' }))
  }
}
watch(
...
const handleConfirm = () => {
  updateFormRef.value?.validate(
    assertFormValidate(() => {
      api.agentLevelController.save({ body: updateForm.value }).then(async (res) => {
  ...
  <div class="update-form">
    <el-form labelWidth="120" class="form" ref="updateFormRef" :model="updateForm" :rules="rules">
      <el-form-item label="佣金比例" prop="rate">
        <el-input-number v-model="updateForm.rate"></el-input-number>
      </el-form-item>
