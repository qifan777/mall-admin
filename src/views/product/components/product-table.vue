...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="价格" prop="price" sortable="custom" width="80" show-overflow-tooltip>
        <template v-slot:default="{ row }: ProductScope">
          {{ row.price }}
        </template>
      </el-table-column>
      <el-table-column label="封面" prop="image" sortable="custom" width="80">
        <template v-slot:default="{ row }: ProductScope">
          <el-avatar :src="row.image" alt=""></el-avatar>
        </template>
      </el-table-column>
      <el-table-column label="品牌" prop="brand" sortable="custom" width="80" show-overflow-tooltip>
        <template v-slot:default="{ row }: ProductScope">
          {{ row.brand }}
        </template>
      </el-table-column>
      <el-table-column label="库存" prop="stock" sortable="custom" width="80" show-overflow-tooltip>
        <template v-slot:default="{ row }: ProductScope">
          {{ row.stock }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="特性" prop="features" sortable="custom">
        <template v-slot:default="{ row }: ProductScope">
          {{ row.features }}
        </template>
      </el-table-column>
      <el-table-column label="规格" prop="specs" sortable="custom">
        <template v-slot:default="{ row }: ProductScope">
          {{ row.specs }}
        </template>
      </el-table-column>
