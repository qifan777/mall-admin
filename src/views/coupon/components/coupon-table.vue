...
import CouponGift from './coupon-gift.vue'
...
const activeGiftCouponId = ref('')
const couponGiftRef = ref<InstanceType<typeof CouponGift>>()
const handleGift = (row: { id: string }) => {
  activeGiftCouponId.value = row.id
  couponGiftRef.value?.openDialog()
}
</script>
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="优惠券类型" prop="couponType" sortable="custom">
        <template v-slot:default="{ row }: CouponScope">
          <dict-column :dict-id="1003" :value="row.couponType"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="使用品类" prop="scopeType" sortable="custom">
        <template v-slot:default="{ row }: CouponScope">
          <dict-column :dict-id="1004" :value="row.scopeType"></dict-column>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" width="120">
        <template v-slot:default="{ row }">
          <div>
            ...
            <ElPopover content="赠送">
              <template #reference>
                <ElButton type="primary" size="small" link @click="handleGift(row)">
                  <el-icon><Present /></el-icon>
                </ElButton>
              </template>
            </ElPopover>
            <coupon-gift :coupon-id="activeGiftCouponId" ref="couponGiftRef"></coupon-gift>
          </div>
        </template>
      </el-table-column>
    </el-table>
