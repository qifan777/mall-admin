...
import type { CouponUserInput } from '@/apis/__generated/model/static'
import { Delete, Edit, Plus } from '@element-plus/icons-vue'
import DictColumn from '@/components/dict/dict-column.vue'

type CouponUserScope = Scope<CouponUserDto['CouponUserRepository/COMPLEX_FETCHER']>
...
const handleCreate = () => {
  openDialog('CREATE')
  createForm.value = { ...couponUserStore.initForm }
}
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.couponUserController.delete({ body: ids }).then((res) => {
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="获得渠道" prop="receiveType" sortable="custom">
        <template v-slot:default="{ row }: CouponUserScope">
          <dict-column :dict-id="1006" :value="row.receiveType"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="优惠券" prop="coupon" sortable="custom">
        <template v-slot:default="{ row }: CouponUserScope">
          {{ row.coupon.name }}
        </template>
      </el-table-column>
      <el-table-column label="拥有者" prop="user" sortable="custom">
        <template v-slot:default="{ row }: CouponUserScope">
          {{ row.user.nickname }}({{ row.user.phone }})
        </template>
      </el-table-column>
      <el-table-column label="优惠券状态" prop="status" sortable="custom">
        <template v-slot:default="{ row }: CouponUserScope">
          <dict-column :dict-id="1005" :value="row.status"></dict-column>
        </template>
      </el-table-column>
