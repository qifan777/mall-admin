...
import type { Page, ProductCategoryInput, ProductCategorySpec, QueryRequest } from '../model/static'

export class ProductCategoryController {
  constructor(private executor: Executor) {}

  async delete(options: ProductCategoryControllerOptions['delete']): Promise<boolean> {
  ...
  ): Promise<Page<ProductCategoryDto['ProductCategoryRepository/COMPLEX_FETCHER']>> {
    const _uri = '/productCategory/query'
    return (await this.executor({ uri: _uri, method: 'POST', body: options.body })) as Page<
      ProductCategoryDto['ProductCategoryRepository/COMPLEX_FETCHER']
    >
  }

  async save(options: ProductCategoryControllerOptions['save']): Promise<string> {
    const _uri = '/productCategory/save'
    return (await this.executor({ uri: _uri, method: 'POST', body: options.body })) as string
  }
}
